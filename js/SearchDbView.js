...
function buildProfesionalesUrl(page = 1) {
  ...
  url.searchParams.set("limit", DB_LIMIT);
...
function renderResults(data) {
  ...
  data.forEach((profesional) => {
    ...
    const profesion = profesional.profesion ?? "Sin profesión";
...
function getFilterSummary() {
  ...
  if (profesionInput.value.trim()) filtros.push(`Profesión: ${profesionInput.value.trim()}`);
